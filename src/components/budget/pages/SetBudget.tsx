...
const SetBudget = () => {
  ...
  const total = useSelector<AppState, number>((state) => state.budget.totalBudget);

  const submitHandle = async (data: FieldValues) => {
    const { totalBudget,expenseBudget,investmentBudget,savingsBudget } = data;
    if(!totalBudget){
  ...
  return (
    <div className="row bg-indigo-200">
      ...
      <div className="lg:w-3/4 sm:w-3/5 my-4 m-4 p-4 bg-white rounded-xl flex align-center justify-around ">
          ...
          <Form className="flex-col lg:w-2/3 sm:w-2/2" onSubmit={handleSubmit(submitHandle)}>
            ...
            <div className="row m-4 align-center justify-center flex">
              ...
              <Controller                
                ...
                // defaultValue={total}
                render={({ field }) => (
                  <input className='font-bold text-black col-6 border-1 text-xs m-1' type="text" {...field} placeholder={`${total}`}/>
                )}
              />
            </div>
            <div className="row m-4 align-center justify-center flex">
              ...
              <Controller           
                ...
                defaultValue={50}
                render={({ field }) => (
                  <input className='col-6 border-1 text-xs m-1 font-bold' type="text" {...field} placeholder='50'/>
                )}
              />%
            </div>
            <div className="row m-4 align-center justify-center flex">
              ...
              <Controller                
                ...
                defaultValue={30}
                render={({ field }) => (
                  <input className='col-6 border-1 text-xs m-1 font-bold' type="text" {...field} placeholder='30'/>
                )}
              />%
            </div>
            <div className="row m-4 align-center justify-center flex">
              ...
              <Controller                
                ...
                defaultValue={20}
                render={({ field }) => (
                  <input className='col-6 border-1 text-xs m-1 font-bold' type="text" {...field} placeholder='20'/>
                )}
              />%
            </div>
